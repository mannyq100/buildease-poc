...
export class MediaPermissionService {
  ...
  canSetAsProfile(item: MediaItem): boolean {
    ...
    // Only images can be set as profile
    if (item.type !== 'image' && item.type !== 'progress-image') return false;
    
    // Owner can set any image as profile
    if (this.context.isOwner) return true;
   ...
   */
  canUpload(type: 'inspiration' | 'progress' | 'documents'): boolean {
    if (!this.context.user) return false;
    
    // Owner can upload anything
    if (this.context.isOwner) return true;
    
    // Team members can upload progress images and documents
    if (this.context.isTeamMember) {
      return type === 'progress' || type === 'documents';
    }
...
export const useMediaPermissions = (
  ...
  return {
    ...
    canUpload: (type: 'inspiration' | 'progress' | 'documents') => service.canUpload(type)
  };
};
