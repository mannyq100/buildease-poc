...
export const useOptimizedFiltering = (
  ...
  return useMemo(() => {
    return items.filter(item => {
      ...
      // Type filter with optimized logic
      if (filters.type !== 'all') {
        if (filters.type === 'image' && !['image', 'progress-image'].includes(item.type)) {
          return false;
        }
        if (filters.type === 'document' && item.type !== 'document') {
          return false;
        }
        if (filters.type === 'progress-image' && item.type !== 'progress-image') {
          return false;
        }
      }
